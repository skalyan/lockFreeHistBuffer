...
public:
	...
	push(T *x)
	{
		...
		ptr_array_[(start_ + count_) % Q_SIZE] = x;
      if (IsFull()) {
         start_ = (start_ + 1) % Q_SIZE;
      } else {
         ++count_;
      }
	}
	...
	T *
	pop(unsigned long i = 0)
	{
		boost::shared_lock<boost::shared_mutex> lock(mtx_);

		T *x = nullptr;
      if (count_ > 0) {
		   x = ptr_array_[(start_ + count_ - 1 - i) % Q_SIZE];
		}
		return x;
	}

private:
   unsigned long endIndex(unsigned long i) {
		return (start_ + count_ - 1) % Q_SIZE;
   }
	bool IsFull() {
		return count_ == Q_SIZE;
	}
...
public:
		...
		last_tail_(0)
	{
		...
		// KAL thr_p_ = (ThrPos *)::memalign(getpagesize(), sizeof(ThrPos) * n);
		assert(thr_p_);
		// Set per thread tail and head to ULONG_MAX.
		::memset((void *)thr_p_, 0xFF, sizeof(ThrPos) * n);

		posix_memalign((void**)&ptr_array_, getpagesize(), Q_SIZE * sizeof(void *));
		// KAL ptr_array_ = (T **)::memalign(getpagesize(), Q_SIZE * sizeof(void *));
		assert(ptr_array_);
	}
